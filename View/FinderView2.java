 ...
 * @author daizhiqing
 */
public class FinderView2 extends View {
    ...
    public Rect middleRect = new Rect();  //扫码框的矩形
    ...
    public FinderView2(Context context) {
        super(context);
        init(context);
    }

    public FinderView2(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    private void init(Context context) {
        int finder_mask = context.getResources().getColor(R.color.zxing_custom_viewfinder_mask);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        middleRect.set((measureedWidth - borderWidth) / 2,
                (measureedHeight - borderWidth) / 2,      // 上面距离
                (measureedWidth - borderWidth) / 2 + borderWidth,  //右边
                (measureedHeight - borderWidth) / 2 + borderWidth); //下面
        lineRect.set(middleRect);
        lineRect.bottom = lineRect.top + lineHeight;
